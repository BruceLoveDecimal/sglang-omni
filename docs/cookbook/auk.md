...
Two further sampling-speed options are on by default. `--auk_engine.factory.enable_dit_torch_compile` compiles each transformer block once for all blocks of its kind, folding the modulation, gating, and normalization chain into the matmul stream; that takes one step from about 2400 kernel launches to 600. `--auk_engine.factory.enable_dit_cuda_graph` captures one whole Euler step as a CUDA graph and replays it for every NFE step: only the noised latent and the timestep change between steps, so one capture serves a whole trajectory, and every later request whose shape rounds to the same bucket. Measured on one H200 with BF16 weights, NFE=32, and CFG=2.0, in ms per step: compile alone takes batch size 1 from 31.6 to 15.9 and batch size 8 from 71.6 to 38.4; adding the graph reaches 7.8 at batch size 1, i.e. 4.0x over eager. A wide batch is not launch-bound -- the graph moves batch size 8 only from 38.4 to 36.8, less than the padding it needs costs there -- so batches above two run the step eagerly and leave the cache to the shapes that gain from it. End to end on 48 SeedTTS-en voice-clone requests, the two together take p95 from 1.68s to 1.09s at one concurrent request and throughput from 1.69 to 3.06 requests per second at 64. Set either to `false` to fall back.

Graph capture requires padding, so target frames, reference frames, and text tokens round up to multiples of 32, 64, and 64. Buckets that coarse are what keep a varied corpus on a handful of graphs: over a 288-request SeedTTS sweep, 16/16/8 asked for 61 shapes and most of them never got one, while these ask for 25 and all are captured. Padded rows are computed and discarded, not blended in: the attention bias sends masked keys to `-inf`, the convolutional position embedding masks its own input, and rope positions come from the real lengths, so a padded batch produces what the unpadded one does. `enable_dit_cuda_graph` is refused on a checkpoint whose config disables `attn_mask_enabled`, since that bias is what makes padding neutral. The costs: block compilation adds about 15s to startup, each new shape costs 0.2-0.5s to capture and 50MiB of graph pool at SeedTTS lengths (175MiB at 1500 frames), drawn from memory the process has already reserved once the server is warm, and past `max_graphs` shapes (32) or below 4GB of free memory the runner logs a warning and runs that shape eagerly. A capture happens on the request that first needs its shape, so a fresh server pays them out of its first minute of traffic: on 48 SeedTTS requests at c=8, p95 was 11.9s while the cache filled and 2.9s once it had. Sampling stays reproducible for a fixed input shape; a different padded width is a different numerical path, and the sampler is sensitive enough that any such change moves the latent measurably.
Multi-request DiT batches can opt in to packed execution with
`--auk_engine.factory.enable_packed_dit true`. This requires a CUDA BF16
backbone and a checkpoint with `attn_mask_enabled`. The transformer blocks run
only valid text/reference/target tokens through Linear, Norm, FFN and
non-causal variable-length FlashAttention. Original position IDs and separate
CFG branches are preserved. Convolutional audio embeddings and the FP32 Euler
state retain their padded layout; layout indices are built once per trajectory.
Singleton batches retain the existing execution path. The option defaults to
false. It uses SGLang FA4 on Blackwell and FA3 on earlier CUDA devices, subject
to backend availability; it does not enable CUDA graph capture. Different
attention and GEMM reductions can change generated waveforms, so validate audio
quality and serving performance on the intended workload before enabling it.
