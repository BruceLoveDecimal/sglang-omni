...
class BoundStep(ShapePadding):
    """A runner that binds: the step runs from the flat mapping it was given,
    the way a capture replays from its static buffers."""

    def __init__(self):
        self.inputs = None

    def bind(self, step, inputs, *, x, time, baked):
        self.inputs = inputs
        return lambda t, x: step(inputs, t, x)


def test_a_bound_step_integrates_the_flattened_plan(monkeypatch):
    """The plan crosses the runner as tensors and scalars only, and comes
    back as the trajectory the eager step integrates."""
    from sglang_omni.models.auk.dit import AuKDit
    from sglang_omni.models.auk.flow_matching import AuKFlowMatching, AuKSampleItem
    ...
    items = [
        ...
        AuKSampleItem(torch.randn(8, 16), torch.ones(8, dtype=torch.bool), 11, seed=2),
    ]
    sampling = dict(steps=3, cfg_strength=2.0)
    expected = flow.sample_batch(items, **sampling, step_graph=ShapePadding())
    bound = BoundStep()
    actual = flow.sample_batch(items, **sampling, step_graph=bound)

    assert all(
        isinstance(value, (torch.Tensor, bool, int, float)) or value is None
        for value in bound.inputs.values()
    )
    for output, reference in zip(actual, expected):
