...
def create_auk_engine_executor(
    ...
    *,
    ...
    enable_packed_dit=None packs the multi-request batches no captured graph
    covers wherever the device, weight dtype, checkpoint and FlashAttention
    build allow it and logs the reason when it falls back to the padded path;
    True makes the same checks fatal and False never packs.
    """
    ...
    # Resolved before load_flow so an unusable device, checkpoint or kernel is
