...
@torch.library.custom_op("sglang_omni::auk_packed_flash", mutates_args=())
def _packed_flash(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    # Keep backend dispatch/JIT out of Dynamo while retaining block fusion.
...
@_packed_flash.register_fake
def _packed_flash_fake(q, k, v, cu_seqlens, max_seqlen, version):
    return torch.empty(q.shape, dtype=q.dtype, device=q.device)


def flash_attention(q, k, v, layout):
    return _packed_flash(
        q, k, v, layout.cu_seqlens, layout.max_seqlen, layout.flash_version
    )
