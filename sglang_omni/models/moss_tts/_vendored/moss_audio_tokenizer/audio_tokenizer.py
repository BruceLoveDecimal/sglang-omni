...
#
# Vendored from OpenMOSS-Team/MOSS-Audio-Tokenizer via vllm-omni
# (vllm_omni/model_executor/models/moss_tts/audio_tokenizer.py, commit b550709b).
# See README.md in this directory for full provenance.
#
# Local modifications relative to the vllm-omni copy: streaming KV-cache decode
# re-introduced (the vllm-omni copy was single-pass batch decode only). The
# non-streaming batch_encode / batch_decode path is behaviourally unchanged.
"""MOSS Audio Tokenizer — codec with batch decode + stateful streaming decode."""
...
@dataclass
class LayerKV:
    """Post-RoPE K/V cache for one attention layer. Shapes: ``[B, H, T, D]``."""
...
@dataclass
class StageCache:
    """KV cache for one decoder transformer *stage* (one ``_ProjectedTransformer``).

    ``pos_offset`` is the RoPE position offset *at this stage's time rate*. Stages
    run at different rates because patch upsampling changes T between them, so each
    stage keeps its own offset (RFC §5.1).
    """
...
class StreamingCache:
    """Per-request streaming codec state: one ``StageCache`` per decoder stage."""
...
def _apply_rope(
    q: torch.Tensor, k: torch.Tensor, max_period: float = 10_000, offset: int = 0
) -> tuple[torch.Tensor, torch.Tensor]:
    """Rotary position embedding over sequence dim (B, H, T, D).

    ``offset`` shifts the absolute positions so RoPE continues across streaming
    chunks: positions are ``arange(offset, offset+T)``. With ``offset=0`` this is
    identical to the non-streaming path.

    Matches upstream MossAudioTokenizer's ``apply_rope``: pair the last dim as
    ``(re, im)`` interleaved (GPT-J style) — *not* GPT-NeoX split-halves. The two
    conventions are not interchangeable with the same checkpoint.
    """
    B, H, T, D = q.shape
    half = D // 2
    ds = torch.arange(half, device=q.device, dtype=torch.float32)
    freqs = torch.exp(ds * (-math.log(max_period) * 2 / D))
    ts = torch.arange(offset, offset + T, device=q.device, dtype=torch.float32).view(1, 1, -1, 1)
...
class _Attention(nn.Module):
    """Causal multi-head self-attention with RoPE and optional streaming KV cache."""
    ...
    def forward(self, x: torch.Tensor, kv: LayerKV | None = None) -> torch.Tensor:
        B, T, _ = x.shape
        ...
        if kv is None:
            # Non-streaming batch path — behaviourally identical to upstream.
            q, k = _apply_rope(q, k, self.max_period)
            out = F.scaled_dot_product_attention(q, k, v, is_causal=self.causal)
            out = out.transpose(1, 2).reshape(B, T, self.embed_dim)
            return self.out_proj(out)

        # Streaming path. ``offset`` = number of frames already cached at this
        # stage; it is exactly the cached K length, which equals the stage's
        # ``pos_offset``. RoPE for the new frames continues from there; cached K
        # already carries its own absolute-position rotation.
        offset = int(kv.k.shape[2]) if kv.k is not None else 0
        q, k = _apply_rope(q, k, self.max_period, offset=offset)
        if kv.k is not None:
            k = torch.cat([kv.k, k], dim=2)
            v = torch.cat([kv.v, v], dim=2)
        kv.k = k
        kv.v = v

        if self.causal:
            t_total = int(k.shape[2])
            idx_q = torch.arange(T, device=x.device).view(-1, 1) + offset
            idx_k = torch.arange(t_total, device=x.device).view(1, -1)
            mask = idx_k <= idx_q  # (T, t_total) bool; True = attend
...
class _TransformerLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv: LayerKV | None = None) -> torch.Tensor:
        x = x + self.ls1(self.attn(self.norm1(x), kv=kv))
...
class _Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, stage_cache: StageCache | None = None) -> torch.Tensor:
        if stage_cache is None:
            for layer in self.layers:
                x = layer(x)
            return x
        # Streaming: one LayerKV per layer, created lazily.
        while len(stage_cache.layer_kvs) < len(self.layers):
            stage_cache.layer_kvs.append(LayerKV())
        for i, layer in enumerate(self.layers):
            x = layer(x, kv=stage_cache.layer_kvs[i])
        return x
...
class _ProjectedTransformer(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, lengths: torch.Tensor, stage_cache: StageCache | None = None
    ) -> tuple[torch.Tensor, torch.Tensor]:
        x = self.in_proj(x.transpose(1, 2))  # (B, D, T) → (B, T, d_model)
        x = self.transformer(x, stage_cache=stage_cache)
        x = self.out_proj(x).transpose(1, 2)  # (B, T, D) → (B, D, T)
        return x, lengths


class _PatchedPretransform(nn.Module):
    """Patch-based up/down-sampling (no learned weights, no cross-time mixing)."""
...
class _ResidualQ(nn.Module):
    ...
    def decode_codes(self, codes: torch.Tensor) -> torch.Tensor:
        """codes: (NQ, B, T) → (B, rvq_dim, T). Per-frame, no temporal state."""
        nq, B, T = codes.shape
...
class MossAudioTokenizerModel(PreTrainedModel):
    """MOSS Audio Tokenizer — batch_encode / batch_decode + streaming decode_stream."""
    ...
    def batch_decode(
        ...
        """Decode a list of (NQ, T) code tensors → waveforms (single pass)."""
    ...
    def decode_stream(
        ...
        state: StreamingCache | None = None,
        last_chunk: bool = False,
        num_quantizers: int | None = None,
    ) -> tuple[MossAudioTokenizerDecoderOutput, StreamingCache]:
        """Stateful streaming decode for a single stream (B=1).

        ``codes_chunk`` is ``(NQ, T_new)`` — only the newly de-delayed frames.
        Returns the waveform for exactly these new frames plus the updated state.
        Concatenating the per-chunk audio equals ``batch_decode`` of the full code
        sequence within numerical tolerance (RFC §6), because the only temporal
        mixing is causal attention and the KV cache reproduces it exactly.
        """
        if state is None:
            state = StreamingCache()
        device = next(self.parameters()).device
        codes = codes_chunk.to(device=device, dtype=torch.long)
        if codes.dim() != 2:
            raise ValueError(f"decode_stream expects (NQ, T) codes, got {tuple(codes.shape)}")
        nq = num_quantizers or codes.shape[0]
        codes = codes[:nq].unsqueeze(1)  # (NQ, 1, T)

        if codes.shape[-1] == 0:
            empty = torch.zeros((1, 1, 0), device=device, dtype=torch.float32)
            if last_chunk:
                state.finished = True
            return MossAudioTokenizerDecoderOutput(audio=empty, audio_lengths=torch.zeros(1, device=device)), state

        z = self.quantizer.decode_codes(codes)  # (1, rvq_dim, T)
        d = z
        d_len = torch.tensor([codes.shape[-1]], device=device, dtype=torch.long)

        stage_idx = 0
        for m in self.decoder:
            if isinstance(m, _ProjectedTransformer):
                while len(state.stage_caches) <= stage_idx:
                    state.stage_caches.append(StageCache())
                stage_cache = state.stage_caches[stage_idx]
                t_in = int(d.shape[-1])
                d, d_len = m(d, d_len, stage_cache=stage_cache)
                stage_cache.pos_offset += t_in
                stage_idx += 1
            else:
                d, d_len = m(d, d_len)
        ...
        if last_chunk:
            state.finished = True
        return MossAudioTokenizerDecoderOutput(audio=d, audio_lengths=d_len), state
